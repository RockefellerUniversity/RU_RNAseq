...
```{r, include=FALSE,echo=toMessage, wanring=F, message=F}
library(ShortRead)
library(Rsamtools)
...
```{r instaaxscslldebbBSccdxxed,cache=TRUE,eval=F,dependson="instalcscldcsceBSded",echo=toMessage}
...
```{r instaaxscslldebbBSccdxxed2,cache=TRUE,eval=TRUE,dependson="instalcscldcsceBSded",echo=F}
fastqFilt <- fastq[readQualityScores >= 1000]

if(!file.exists("data/filtered_ENCFF070QMF.fastq.gz")){
writeFastq(fastqFilt,"data/filtered_ENCFF070QMF.fastq.gz")
}
