...
### 1. Compare 3 datasets
Load the **data/gC_TissueFull.RData** and create a DESeq2 object. Create a venn diagram of genes upregulated in Kidney versus Liver and Heart.

```{r, include=FALSE,warning=FALSE,echo=F}
library(DESeq2)
library(tximport)
...
### 2. KEGG enrichment
Perform an enrichment analysis for genes upregulated in Kidney vs both liver and Heart. Use KEGG pathways and plot the -log10 pvalue of the top 10 terms as a scatter plot.

*Kegg.db is now deprecated so this will not work on R4.1+*
```{r b,cache=TRUE,eval=F,dependson="a",warning=FALSE,echo=toMessage}
library(KEGG.db)
...
### 3. KEGG enrichment (cont)
Create a boxplot plot of log2FC of genes (as from Kidney vs Liver and Kidney vs Heart comparison) in collecting duct acid secretion term vs all genes not in duct acid secretion term.
*Kegg.db is now deprecated so this will not work on R4.1+*
```{r c,cache=TRUE,eval=F,dependson="b",warning=FALSE,echo=toMessage}
...
### 4. Subset to interesting genes
Identify genes changing significantly in any condition (< 0.05). Produce a file of results containing gene names.
