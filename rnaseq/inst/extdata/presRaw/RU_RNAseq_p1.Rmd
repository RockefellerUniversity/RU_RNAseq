...
## Raw RNAseq data QC
We can plot the abundance of DNA bases (A,C,G,T) over the length of read to see any biases.
```{r tregFQ3_split,cache=TRUE,dependson="tregFQ2",fig.height=3,fig.width=7}
...
```{r tregFQ4_split,cache=TRUE,dependson="tregFQ0",fig.height=2,fig.width=7}
...
There is no R package for Salmon. It is possible to install it on Mac and Linux using [Anaconda](https://anaconda.org/bioconda/salmon) package repository. Anaconda is a huge collection of version controlled packages that can be installed through the conda package management system. With conda it is easy to install and manage environments that have a variety of different packages in them. 

Though there is no Salmon R package, we can control the anaconda package system using the R package [Herper](https://bioconductor.org/packages/release/bioc/html/Herper.html). This was created by us here at the BRC and is part of Bioconductor.
...
```{r sal2t,echo=F,eval=FALSE,cache=TRUE,dependson="sal1"}
mainChromosomes <- paste0("chr",c(1:19,"X","Y","M"))
mainChrSeq <- lapply(mainChromosomes,
                     function(x)BSgenome.Mmusculus.UCSC.mm10[[x]])
names(mainChrSeq) <- mainChromosomes
