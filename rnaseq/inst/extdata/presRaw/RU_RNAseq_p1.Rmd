...
There is no R package for Salmon. It is possible to install it on Mac and Linux using the [Anaconda](https://anaconda.org/bioconda/salmon) package repository. Anaconda is a huge collection of version controlled packages that can be installed through the conda package management system. With conda it is easy to create and manage environments that have a variety of different packages in them. 
...
```{r sal2t,echo=F,eval=T,cache=TRUE,dependson="sal1",  cache.lazy = FALSE}
mainChromosomes <- paste0("chr",c(1:19,"X","Y","M"))
mainChrSeq <- lapply(mainChromosomes,
                     function(x)BSgenome.Mmusculus.UCSC.mm10[[x]])
names(mainChrSeq) <- mainChromosomes
...
```{r salI_1,echo=TRUE,eval=T,dependson="sal1", warning=F}
...
```{r salI_1t,echo=TRUE,eval=T,dependson="sal1"}
