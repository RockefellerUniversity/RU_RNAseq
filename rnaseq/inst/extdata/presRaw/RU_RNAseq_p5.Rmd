...
```{r de222z,eval=TRUE,echo=TRUE,cache=F,dependson="de22z",warning=FALSE,message=FALSE}
library(DEXSeq)
...
```{r de222zTT,include=FALSE,cache=F,dependson="de222z",}
mcols(senescence_ExonCounts)$gene_id <- unlist(mcols(senescence_ExonCounts)$gene_id)
```

```{r de2223z,eval=TRUE,echo=TRUE,cache=F,dependson="de222zTT",warning=FALSE,message=FALSE}
...
```{r de223l,eval=TRUE,echo=TRUE,cache=F,dependson="de2223z",warning=FALSE,message=FALSE}
...
<!-- For this we will be using Tissue specific RNAseq datasets from ENCODE.  -->
...
<!-- When we load in the data object we have a **RangedSummarizedExperiment** which contains counts from three groups. -->
