...
```{r func1,eval=TRUE,echo=TRUE,cache=TRUE,dependson="func", warning=F, message=F, fig.height=3,fig.width=3}
library(goseq)
...
First we need a geneset. Lets look at Msigdb enrichment of C7 (immunological signature). Clusterporfiler needs a 2 column dataframe with the geneset names and gene IDs. 
...
The result is special object which contains your result. When you look at what is inside you get a preview of what test was done and what the results look like. You can also jconvert this to a dataframe and export.
...
There are a couple of easy functions to visualize the top hits in your result. For example this dotplot which shows amount of overlap with geneset.

These are also all built in ggplot2, so it is easy to modify plotting parameters. For more info on ggplot2 check our course [here](https://rockefelleruniversity.github.io/Plotting_In_R/presentations/singlepage/ggplot2.html).
