...
We can also have access to information on the gene sets themselves using some of the bioconductor resources including **GO.db**, **KEGG.db**, **reactome.db** packages.
...
We can read in gene sets in the gmt format using the GSEABase package.
...
## Human to mouse gene sets

To make use of human gene sets in mouse we can either
...
An alternative is that we translate our human MSigDB collection to mouse gene set, and throw away terms specific to human (such as chromosome location).
...
This contains a named list gene sets converted to mouse Entrez IDs.
...
```{r func1,eval=TRUE,echo=TRUE,cache=TRUE,dependson="func", warning=F, message=F}
library(goseq)
