...
```{r dr1,eval=TRUE,echo=FALSE,cache=TRUE,dependson=c("de7")}
DESeq2::summary(myRes)
```

```{r drssaas,eval=FALSE,echo=TRUE,cache=FALSE}
summary(myRes)
```
...
```{r dr1b,eval=TRUE,echo=TRUE,cache=TRUE,dependson=c("de7"),fig.height=4,fig.width=7,message=FALSE,warning=FALSE}
...
```{r anno,eval=TRUE,echo=TRUE,cache=TRUE,dependson=c("tx7","de7"),message=FALSE,warning=FALSE,tidy=FALSE}
library(org.Mm.eg.db)
...
```{r anno2,eval=TRUE,echo=TRUE,cache=TRUE,dependson="anno",tidy=FALSE}
