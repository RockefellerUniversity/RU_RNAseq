---
title: "Analysis of RNAseq data in R and Bioconductor (part 2)<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>"
...
In this session we will import our expression estimates into R, and then identify differences in gene expression between conditions. 
...
Here we will handle multiple files. First I am using a **BamFileList** of all the T-Reg samples to easily organize my samples and save memory with the **yieldSize** parameter.
...
The DESeq2 package contains a workflow for assessing local changes in fragment/read abundance between replicated conditions. This workflow includes normalization, variance estimation, outlier removal/replacement as well as significance testing suited to high throughput sequencing data (ie. integer counts).
...
```{r de22,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de1", warning=F}
...
```{r de3,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de1", warning=F}
...
```{r de6,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de4",fig.width=6,fig.height=4}
plotDispEsts(dds)
```
...
```{r tx6,eval=TRUE,echo=TRUE,cache=TRUE,dependson="tx5", warning=F}
