...
```{r gC1,eval=TRUE,echo=FALSE,cache=FALSE}
load("../../Data/senescence_ExonCounts.RData")
# rownames(senescence_ExonCounts) <- paste0(unlist(mcols(rowRanges(senescence_ExonCounts))$gene_id),"_",unlist(mcols(rowRanges(senescence_ExonCounts))$exonic_part))
# save(senescence_ExonCounts,file="../../Data/senescence_ExonCounts.RData")
senescence_ExonCounts
```
...
```{r de222,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de22",warning=FALSE,message=FALSE}
...
```{r de2223,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de222",warning=FALSE,message=FALSE}
...
```{r de223,eval=TRUE,echo=TRUE,cache=TRUE,dependson="de2223",warning=FALSE,message=FALSE}
ToFilter <- apply(assay(senescence_ExonCounts),1,function(x)sum(x > 10)) > 2
tregExonCounts <- senescence_ExonCounts[ToFilter,]
table(ToFilter)
ddx <- DEXSeqDataSetFromSE(senescence_ExonCounts,
                     design= ~ sample + exon + condition:exon)
ddx
...
```{r da5,eval=TRUE,echo=TRUE,cache=TRUE,dependson="da2",fig.height=5,fig.width=7}
